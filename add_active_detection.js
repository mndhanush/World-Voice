...
// 1. Get the base navbar from curriculum.html (which currently has Curriculum as active)
...
// We'll replace all teal-primary dark:text-teal-primary bg-gray-50 dark:bg-gray-800 instances
// But we must be careful not to reset the logo or other things!
// Let's just find and reset the specific active ones that we know might exist from our previous script.

// Reset Desktop active state to normal
...
// Make sure to preserve the xl: breakpoints we just fixed!
// Base navbar already has xl:flex and xl:hidden because curriculum.html was updated.
...
// Remove any existing active detection code just in case
if (menuScript.includes('// Active Page Detection')) {
    menuScript = menuScript.substring(0, menuScript.indexOf('// Active Page Detection'));
}
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      desktopLinks.forEach(link => {
          if (link.getAttribute('href') === currentPath) {
              ...
              link.classList.remove('text-gray-800', 'dark:text-gray-200', 'hover:bg-gray-100', 'dark:hover:bg-gray-800');
              // Add active classes
...
menuScript = menuScript.replace('  </script>\n</body>\n</html>', activeDetectionJS);
...
for (const file of files) {
    ...
    let fScriptStart = content.indexOf('  <script>\n    // Theme Toggle');
    if (fScriptStart !== -1) {
